...
```
<br>
organization * unique items interaction:
````{r}
...
organization * seq length
```{r}
post.rt.mod2<-lmer(SearchTimeAvg~SeqType + (1|SUBJECT), data = e1a[e1a$Organization=="No",], REML=F, na.action=na.omit, contrasts=list(SeqType="contr.sum"))
post.rt.mod3<-lmer(SearchTimeAvg~SeqType + (1|SUBJECT), data = e1a[e1a$Organization=="Yes",], REML=F, na.action=na.omit, contrasts=list(SeqType="contr.sum"))
tab_model(post.rt.mod2, post.rt.mod3, show.stat = T, p.val = "wald", digits.p = 2, digits=3)
```
...
```{r}
moves.mod3a<-glmer(Org.Num ~ trial.c+(1|SUBJECT), data = e1a[e1a$SeqType=="Long",], family = binomial(link = logit)) 
moves.mod3b<-glmer(Org.Num ~ trial.c+(1|SUBJECT), data = e1a[e1a$SeqType=="Short",], family = binomial(link = logit)) 
...
organization * seq length
```{r}
arr.mod1<-lmer(Distance~SeqType+(1|SUBJECT), data = e1a.org.sub[e1a.org.sub$Arrangement=="Pre-Organization",], REML=F, na.action=na.omit, contrasts = list (SeqType="contr.sum"))
arr.mod2<-lmer(Distance~SeqType+(1|SUBJECT), data = e1a.org.sub[e1a.org.sub$Arrangement=="Post-Organization",], REML=F, na.action=na.omit, contrasts = list (SeqType="contr.sum"))

tab_model(arr.mod1, arr.mod2, transform=NULL, show.stat = T, p.val = "wald", digits.p = 2) 
```
<br>
organization * unique items
```{r}
...
```{r}
# seq length * unique items at each level of arrangement
arr.mod2<-lmer(Distance~SeqType*uniq.c+(1|SUBJECT), data = e1a.org.sub[e1a.org.sub$Arrangement=="Pre-Organization", ], 
               REML=F, na.action=na.omit, contrasts = list(SeqType="contr.sum"))

arr.mod3<-lmer(Distance~SeqType*uniq.c+(1|SUBJECT), data = e1a.org.sub[e1a.org.sub$Arrangement=="Unorganized", ], 
               REML=F, na.action=na.omit, contrasts = list(SeqType="contr.sum"))
...
3-way interaction
```{r}
arr.mod2<-lmer(Distance~SeqType*uniq.c+(1|SUBJECT), data = e1a.org.sub[e1a.org.sub$Arrangement=="Unorganized",], 
               REML=F, na.action=na.omit, contrasts = list (SeqType="contr.sum"))
arr.mod3<-lmer(Distance~SeqType*uniq.c+(1|SUBJECT), data = e1a.org.sub[e1a.org.sub$Arrangement=="Post-Organization",], 
               REML=F, na.action=na.omit, contrasts = list (SeqType="contr.sum"))
tab_model(arr.mod2, arr.mod3, transform=NULL, show.stat = T, p.val = "wald", digits.p = 2) 
```
<br>
seq length * unique items for the unorganized trials
```{r}
arr.mod4<-lmer(Distance~uniq.c+(1|SUBJECT), data = e1a.org.sub[e1a.org.sub$Arrangement=="Unorganized" & e1a.org.sub$SeqType=="Short",], REML=F, na.action=na.omit, contrasts = list (SeqType="contr.sum"))
arr.mod5<-lmer(Distance~uniq.c+(1|SUBJECT), data = e1a.org.sub[e1a.org.sub$Arrangement=="Unorganized" & e1a.org.sub$SeqType=="Long",], REML=F, na.action=na.omit, contrasts = list (SeqType="contr.sum"))
tab_model(arr.mod4, arr.mod5, transform=NULL, show.stat = T, p.val = "wald", digits.p = 2) 
```
...
arr.mod2<-lmer(Distance~SeqType+(1|SUBJECT), data = item1a.sub[item1a.sub$Arrangement=="Pre-Movement",], REML=F,
               na.action=na.omit, contrasts = list(SeqType="contr.sum"))

arr.mod3<-lmer(Distance~SeqType+(1|SUBJECT), data = item1a.sub[item1a.sub$Arrangement=="Post-Movement",], REML=F,
               na.action=na.omit, contrasts = list(SeqType="contr.sum"))
...
```
<br>
sequence length * arrangement:
```{r}
# sequence length * arrangement interaction 
arr.mod2<-lmer(Distance~SeqType+(1|SUBJECT), data = item1a.sub[item1a.sub$Arrangement=="Pre-Movement",], REML=F,
               na.action=na.omit, contrasts = list(SeqType="contr.sum"))

arr.mod3<-lmer(Distance~SeqType+(1|SUBJECT), data = item1a.sub[item1a.sub$Arrangement=="Unmoved",], REML=F,
               na.action=na.omit, contrasts = list(SeqType="contr.sum"))
...
<br>
seq length * unique items for unmoved items:
```{r}
# breaking down two-way interaction between seq length and unique items for unmoved items
...
<span style="color:red">
The following analysis is the two-way interaction between arrangement and sequence lengths
</span>


```{r}
arr.mod4<-lmer(Distance~SeqType+(1|SUBJECT), data = item1a.sub[item1a.sub$Arrangement=="Unmoved", ], 
               REML=F, na.action=na.omit, contrasts = list(SeqType="contr.sum"))
arr.mod5<-lmer(Distance~SeqType+(1|SUBJECT), data = item1a.sub[item1a.sub$Arrangement=="Post-Movement", ], 
               REML=F, na.action=na.omit, contrasts = list(SeqType="contr.sum"))
tab_model(arr.mod4, arr.mod5, transform=NULL, show.stat = T, p.val = "wald", digits.p = 3) 
```

<span style="color:red">
The pattern is largely the same as in E1b; item distance to the goal location is further for items in the short than long sequences, but this difference is more pronounced for unmoved items than items after they were moved.
</span>
...
```{r}
# organization * seq length interaction
post.rt.mod2<-lmer(SearchTimeAvg~SeqType + (1|SUBJECT), data = e1b[e1b$Organization=="No",], REML=F,
                   na.action=na.omit, contrasts = list(SeqType="contr.sum"))
post.rt.mod3<-lmer(SearchTimeAvg~SeqType + (1|SUBJECT), data = e1b[e1b$Organization=="Yes",], REML=F,
                   na.action=na.omit, contrasts = list(SeqType="contr.sum"))
...
There was a significant two-way interaction between movement state (pre vs. post) and sequence length (b = 4.53, 95% CI [.92, 8.13], t = 2.46, p = .01). While short search sequences were associated with longer item distances both pre-movement (b = 11.07, 95% CI [4.09, 18.06], t = 3.11, p < .001) and post-movement (b = 4.13, 95% CI [1.81, 6.46], t = 3.48, p < .001), the discrepency in distance was bigger in the former than the latter. 
...
arrangement * unique items
```{r}
arr.mod2<-lmer(Distance~uniq.c+(1|SUBJECT), data = item1b.sub[item1b.sub$Arrangement=="Unmoved",], REML=F, na.action=na.omit)

arr.mod3<-lmer(Distance~uniq.c+(1|SUBJECT), data = item1b.sub[item1b.sub$Arrangement=="Pre-Movement",], REML=F, na.action=na.omit)
tab_model(arr.mod2, arr.mod3, transform = NULL, show.stat = T, p.val = "wald", digits.p = 2)

```

<br>
Overall, items tended to be located further from the goal location on trials with short rather than long search sequences (b = 15.90, 95% CI [10.99, 20.81], t = 6.34, p < .001). 

Items that remained unmoved tended to be placed closer to the goal location than initial placements of items that were later moved (b = -97.88, 95% CI [-103.77, -97.98], t = 32.55, p < .001).

There was a significant two-way interaction between number of unique items and whether items were moved (b = 8.27, 95% CI [2.05, 14.49], t = 2.61, p = .01). Hoever, followup comparisons did not yield significant predictors (all p > .11)
